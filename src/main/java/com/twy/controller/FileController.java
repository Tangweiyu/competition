...
public class FileController {
    ...
    ResponseMessage uploadFile(@RequestParam("multipartFiles") MultipartFile[] multipartFiles) throws IOException {
        ...
        if (multipartFiles != null && multipartFiles.length > 0) {
            for (MultipartFile multipartFile : multipartFiles) {
                ...
                List<com.twy.entity.File> files = fileService.findFileByCompetitionId(competitionId);
                for (com.twy.entity.File file : files) {
                ...
                com.twy.entity.File f = new com.twy.entity.File();
    ...
    ResponseMessage uploadNoticeFile(@RequestParam("multipartFiles") MultipartFile[] multipartFiles) throws IOException {
        ...
        if (multipartFiles != null && multipartFiles.length > 0) {
            for (MultipartFile multipartFile : multipartFiles) {
                String fileName = multipartFile.getOriginalFilename();
                List<com.twy.entity.File> files = fileService.findFileByNotificationId(notificationId);
                //遍历文件，判断文件是否存在
                for (com.twy.entity.File file : files) {
                    if (fileName.equals(file.getFileName())) {
                        tag = true;
    ...
    ResponseMessage deleteFile(String fileId) throws IOException {
        try {
            com.twy.entity.File file = fileService.findFileById(fileId);
    ...
    public @ResponseBody
    ResponseMessage downloadFile(@RequestBody com.twy.entity.File file, HttpServletResponse response) {
        //获取文件的信息
        try {
            com.twy.entity.File dbFile = fileService.findFileById(file.getFileId());
            //拿到文件路径
